...
public Object state ;     // defines the state-space
public Object oper;       // operator used to generate this node
...
public boolean expanded ; // indicates if node has been expanded
boolean tested ;   // indicates if node was ever tested 
float cost=0 ;     // cost to get to this node  
public SearchNode parent; // Node parent
...
public void addLink(Vector<SearchNode> node) {
           for (int i=0 ; i < node.size() ; i++) {
              links.addElement(node.get(i)) ; 
           }
}
...
public void trace() {
   ...
   System.out.println(indent + "Searching " + depth + ": " + label) ;                        
}
